...
```mermaid
flowchart LR
    subgraph init["Этап 1: Инициализация"]
        A[Запуск] --> B[Инициализация MPU-6050]
        B --> C[Инициализация TMC2208]
        C --> D[Калибровка гироскопа]
        D --> E[Сброс PID]
    end
    
    subgraph loop["Этап 2: Основной цикл ~200 Гц"]
        F[Чтение MPU-6050] --> G[Коррекция bias]
        G --> H[Интегрирование → roll, pitch, yaw]
        H --> I[PID + Feedforward]
        I --> J[Сигнал на шаговики]
        J --> K[Баланс]
        K -.->|5 мс| F
    end
```

> Альтернатива: [SVG-версия блок-схемы](images/block_diagram.svg) для скачивания.
