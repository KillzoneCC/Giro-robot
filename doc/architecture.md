...
Прошивка укладывается в **32 КБ Flash** Nano; опциональные команды (`G`, `M`, траектория/одометрия) могут быть отключены в сборке — см. [commands.md](commands.md).
...
кроме счётчика `stationaryHold` для защиты от дрейфа оценки скорости при цели 0 м/с (см. `config.h`: `STATIONARY_*`).
